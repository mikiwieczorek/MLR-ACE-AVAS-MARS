...
Form a split sample of 5000 training cases and 3000 validation/test cases of the observations in the training data file (GenreTrain.csv).  Use set.seed(1) BEFORE using the sample command to create the subsets.
...
```{r}
mod.default = rpart(GENRE~.,data=G.train)
prp(mod.default)
```

```{r}
yhat = predict(mod.default,newdata=G.valid,type="prob")
head(yhat)
```


```{r}
yhat = predict(mod.default,newdata=G.valid,type="class")
...
mod2 = rpart(GENRE~.,data=G.train,control=control)
prp(mod2)
```

```{r}
yhat = predict(mod2,newdata=G.valid,type="class")
misclass(yhat,G.valid$GENRE)
```
...
results = crpart.sscv(mod2,G.train$GENRE,data=Gtrain,B=50)
summary(results)
```
...
modx = rpart(GENRE~.,data=G.train,control=control)
prp(modx)
```

```{r}
yhat = predict(modx,newdata=G.valid,type="class")
misclass(yhat,G.valid$GENRE)
```
...
```{r}
results = crpart.sscv(modx,Gtrain$GENRE,data=Gtrain,B=50)
summary(results)
```
...
Fit a basic model:
```{r}
gen.bag = bagging(GENRE~.,data=G.train,coob=T)
gen.bag
```

check it:
```{r}
yhat = predict(gen.bag,newdata=G.valid,type="prob")
head(yhat)
```

```{r}
yhat = predict(gen.bag,newdata=G.valid,type="class")
misclass(yhat,G.valid$GENRE)
```
...
gen.bag2 = bagging(GENRE~.,data=G.train,nbagg=100,coob=T,control=control)
gen.bag2
```


```{r}
yhat = predict(gen.bag2,newdata=G.valid,type="class")
misclass(yhat,G.valid$GENRE)
```
...
Using it:
```{r}
results = bagg.sscv(mod2,Gtrain$GENRE,data=Gtrain,B=5,nbagg=25)
summary(results)
```
...
```{r}
control = rpart.control(minsplit=3,minbucket=2,cp=0,xval=0)
gen.bag2 = bagging(GENRE~.,data=G.train,nbagg=150,coob=T,control=control)
gen.bag2
```
...
```{r}
results = bagg.sscv(mod2,Gtrain$GENRE,data=Gtrain,B=5,nbagg=150)
summary(results)
...
```{r}
control = rpart.control(minsplit=3,minbucket=2,cp=0,xval=0)
gen.bag2 = bagging(GENRE~.,data=G.train,nbagg=200,coob=T,control=control)
gen.bag2
```
...
```{r}
results = bagg.sscv(mod2,Gtrain$GENRE,data=Gtrain,B=5,nbagg=200)
summary(results)
...
gen.bag2 = bagging(GENRE~.,data=G.train,nbagg=175,coob=T,control=control)
gen.bag2
```
...
```{r}
results = bagg.sscv(mod2,Gtrain$GENRE,data=Gtrain,B=5,nbagg=175)
summary(results)
...
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=1,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 1,      importance = T) 
...
```{r}
# yhat = predict(gen.rf,newdata=G.valid)
# misclass(yhat,G.valid$GENRE)
...
Using it:
```{r}
# results = crf.sscv(gen.rf,Gtrain$GENRE,data=Gtrain)
# summary(results)
...
mtry = 2:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=2,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 2,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 3:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=3,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 3,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 4:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=4,importance=T)
# gen.rf

```

Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 4,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 5:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=5,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 5,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 6:
```{r}
gen.rf = randomForest(GENRE~.,data=G.train,mtry=6,importance=T)
gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 6,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 7:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=7,importance=T)
# gen.rf

```

Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 7,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 10:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=10,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 10,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 15:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=15,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 15,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 20:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=20,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 20,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 25:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=25,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 25,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 30:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=30,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 30,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 36:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=36,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 36,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 8:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=8,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 8,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
mtry = 9:
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=9,importance=T)
# gen.rf

```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 9,      importance = T) 
               Type of random forest: classification
                     Number of trees: 500
...
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=7,importance=T, nodesize = 2)
# gen.rf
```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 7,      importance = T, nodesize = 2) 
               Type of random forest: classification
                     Number of trees: 500
No. of variables tried at each split: 7
...
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=7,importance=T, nodesize = 1, ntree = 750)
# gen.rf
```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 7,      importance = T, nodesize = 1, ntree = 750) 
               Type of random forest: classification
                     Number of trees: 750
No. of variables tried at each split: 7
...
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=7,importance=T, nodesize = 1, ntree = 400)
# gen.rf
```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 7,      importance = T, nodesize = 1, ntree = 400) 
               Type of random forest: classification
                     Number of trees: 400
No. of variables tried at each split: 7
...
Maxnodes = 10
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=7,importance=T, nodesize = 1, ntree = 500, maxnodes = 10)
# gen.rf
```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 7,      importance = T, nodesize = 1, ntree = 500, maxnodes = 10) 
               Type of random forest: classification
                     Number of trees: 500
No. of variables tried at each split: 7
...
Maxnodes = 50
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=7,importance=T, nodesize = 1, ntree = 500, maxnodes = 50)
# gen.rf
```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 7,      importance = T, nodesize = 1, ntree = 500, maxnodes = 50) 
               Type of random forest: classification
                     Number of trees: 500
No. of variables tried at each split: 7
...
Maxnodes = 100
```{r}
# gen.rf = randomForest(GENRE~.,data=G.train,mtry=7,importance=T, nodesize = 1, ntree = 500, maxnodes = 100)
# gen.rf
```
Call:
 randomForest(formula = class ~ ., data = G.train, mtry = 7,      importance = T, nodesize = 1, ntree = 500, maxnodes = 100) 
               Type of random forest: classification
                     Number of trees: 500
No. of variables tried at each split: 7
...
Basic Model:
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=200)
# summary(gen.boost)
...
```{r}
# misclass(gen.boost$class,G.train$class)
```
Table of Misclassification
(row = predicted, col = actual)
   y
...
```{r}
# yhat = predict(gen.boost,newdata=G.valid)
# summary(yhat)
...
# results = boost.sscv(gen.boost,Gtrain$class,data=Gtrain,p=0.25,B=25)
...
Trying Freund's
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=200,coeflearn = "Freund" )
# summary(gen.boost)
```
           Length Class   Mode     
formula        3  formula call     
...
Trying Zhu's
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=200,coeflearn = "Zhu" )
# summary(gen.boost)
```
         Length Class   Mode     
formula        3  formula call     
...
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=100,coeflearn = "Freund" )
# summary(gen.boost)
```
          Length Class   Mode     
formula        3  formula call     
...
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=150,coeflearn = "Freund" )
# summary(gen.boost)
```
           Length Class   Mode     
formula        3  formula call     
...
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=150,coeflearn = "Freund", control=rpart.control(minsplit=3,minbucket=2))
...
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=150,coeflearn = "Freund", control=rpart.control(minsplit=3,minbucket=2, cp=0.01))
# 
# summary(gen.boost)
```
           Length Class   Mode     
...
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=100,coeflearn = "Freund", control=rpart.control(minsplit=5,minbucket=4))
# 
# summary(gen.boost)
```
           Length Class   Mode     
...
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=100,coeflearn = "Freund", control=rpart.control(minsplit=12,minbucket=6))
# 
# summary(gen.boost)
```
           Length Class   Mode     
...
```{r}
# gen.boost = boosting(class~.,data=G.train,mfinal=100,coeflearn = "Freund", control=rpart.control(minsplit=12,minbucket=6, maxdepth = 8, xval = 20))
...
c)	Submit your predictions for the genre of the music samples contained the Gtest.csv file.  (4 pts.)
