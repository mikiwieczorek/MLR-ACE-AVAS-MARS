...
#e e)	Using the optimal lambda (bestlam) for both ridge and Lasso regression fit both models and compare the estimated coefficients for the OLS, ridge, and Lasso regressions. Discuss.  (3 pts.)
...
#F f)	Construct a plot of the predicted test y values vs. the actual y test values for both the ridge and Lasso regression models.  Discuss.  (4 pts.)
...
#appear to be similar in terms of the final result. However, we suspect that that this will not be the case upon using a log transformed y into
#the log scale for question i, as algrorithms such as this often perform similarly when given inefficent data, though this simply
#conjecture
...
set.seed(1)
sam = sample(1:length(y4), size = floor(.667*length(y4)), replace = F)


#IB Fit an OLS model for number of applications using the training set, and report the mean RMSEP for the test set.  (4 pts.)


#We will take it out of the log scale as well in order to give results meaning



X = model.matrix(logApps~.,data=College4)[,-1]
logApps = College4$logApps
Xs = scale(X)
College4.temp = data.frame(logApps,Xs)
PA.ols = lm(logApps~Xs,data=College4.temp,subset=sam)
ypred = predict(PA.ols,newdata=College4.temp[-sam])
RMSEP.ols = sqrt(mean((exp(logApps[-sam])-exp(ypred))^2))
RMSEP.ols
...
grid = 10^seq(10,-2,length=200)
ridge.mod4 = glmnet(Xs[sam,],y[sam],alpha=0,lambda=grid)

lasso.mod4 = glmnet(Xs[sam,],y[sam],alpha=1,lambda=grid)
...
#there peers. Moreover, it also appears that more variables are found closer to the middle, meaning that we have found a greater dichotomy
#within the predictors when looking at logApp compared to PctAcc. This could potenitally allow for some more effective models. We also should
#try to keep log lambda below 5
...
#sense given that we are now trying to predict whether or not a application was submitted rather then acceptance, which is likely to be further
#dominated by a singular variable due to the more fluid nature of the response. Still, this level of strength in a single predictor is
#uncommon and should be heavily considered as we continue to through our process. The favored lambda here should be excessivly low.
...
#We once again see some similarities among the three, with variables such as phd, terminal, gradrate, part, and a bunch of other variables
#that would not seem to correlate with the response all being lowered in priority. In addition, all three of these heavily emphasze the acceptance
#variable which makes sense given that if somebody is accepted, they almost assurdely sent in an application. Otherwise, we patterns
#similar to what was desribed on the PctAccept variable in terms of how each model punishes low performing variables. Although, OLS in
#particular is much more restrained in not effectively zeroing out certain variables this time around, with ridge also feeling a bit
#more conservative with how many it effectively zeroes out.
...
plot(y4[-sam],Ridge.pred,xlab="Test y-values",ylab="Predicted Test y-values")


plot(y4[-sam], lasso.pred,xlab="Test y-values",ylab="Predicted Test y-values")


Ridge.pred  = lasso.pred
...
#While all of the models performed well, OLS was left in the dust with its values of roughly 4.5, as both ridge and LASSO
...
#is LASSO, the ridge is close enough to the point that it practically does not matter. The methods are likey to simply be to similar to get a meaningfully different results when fed the same predictors.
#Still, if one were to try some amount of elastic net one may be able to improve these metrics yet. This does not discount
#the power of these models though, as they both are excellent.
